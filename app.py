...
if xml_file is not None:
    try:
        ...
        for det in root.findall('.//nfe:det', ns):
            ...
            # Busca informações de Lote e Validade
            ...
            produtos.append({
                ...
                "Lote XML": lote_val,
                "Validade XML": validade_val,
                "Qtd. Nota Fiscal": int(qtd_float)
            })

        # 4. Ajuste Manual de Lote, Validade e Quantidade de Etiquetas
        st.subheader("📦 Seleção de Itens e Ajustes para Impressão")
        st.info("Campos de Lote e Validade só ficam habilitados para edição quando não forem informados na Nota Fiscal.")
        ...
        for idx, item in enumerate(produtos):
            ...
            col_lote, col_val, col_qtd = st.columns([1.5, 1.5, 1])
            
            # Trava o campo se já existir no XML
            tem_lote_xml = item['Lote XML'] != "N/I"
            tem_val_xml = item['Validade XML'] != "N/I"

            with col_lote:
                val_lote_inicial = item['Lote XML'] if tem_lote_xml else ""
                lote_digitado = st.text_input(
                    label="Lote",
                    value=val_lote_inicial,
                    placeholder="Informe o Lote",
                    disabled=tem_lote_xml,
                    key=f"lote_input_{idx}"
                )
            
            with col_val:
                val_validade_inicial = item['Validade XML'] if tem_val_xml else ""
                validade_digitada = st.text_input(
                    label="Validade",
                    value=val_validade_inicial,
                    placeholder="ex: DD/MM/AAAA",
                    disabled=tem_val_xml,
            ...
            # Define os valores finais para o PDF
            lote_final = item['Lote XML'] if tem_lote_xml else (lote_digitado.strip() if lote_digitado.strip() else "N/I")
            validade_final = item['Validade XML'] if tem_val_xml else (validade_digitada.strip() if validade_digitada.strip() else "N/I")
